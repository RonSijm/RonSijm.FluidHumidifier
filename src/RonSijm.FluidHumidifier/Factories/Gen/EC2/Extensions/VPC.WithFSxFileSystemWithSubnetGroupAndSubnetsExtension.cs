...
using RonSijm.FluidHumidifier.Factories.FSx;
using RonSijm.FluidHumidifier.Factories.Custom.SubnetFactories;

namespace RonSijm.FluidHumidifier.Factories.EC2;

public static class FSxFileSystemSubnetGroupWithSubnetsExtension
{
    public static FileSystemFactory WithFSxFileSystemSubnetGroupAndSubnets(this VPCFactory factory, string subnetName, Action<SubnetFactoryConfig> config)
    {
        var subnets = new List<IResultFactory>();
        var dbSubnetGroupFactory = factory.CreateSubFactory(() => new FileSystemFactory($"{subnetName} Subnet Group", x =>
        {
