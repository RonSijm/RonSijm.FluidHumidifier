...
using RonSijm.FluidHumidifier.Factories.AmazonMQ;
using RonSijm.FluidHumidifier.Factories.Custom.SubnetFactories;

namespace RonSijm.FluidHumidifier.Factories.EC2;

public static class AmazonMQBrokerSubnetGroupWithSubnetsExtension
{
    public static BrokerFactory WithAmazonMQBrokerSubnetGroupAndSubnets(this VPCFactory factory, string subnetName, Action<SubnetFactoryConfig> config)
    {
        var subnets = new List<IResultFactory>();
        var dbSubnetGroupFactory = factory.CreateSubFactory(() => new BrokerFactory($"{subnetName} Subnet Group", x =>
        {
