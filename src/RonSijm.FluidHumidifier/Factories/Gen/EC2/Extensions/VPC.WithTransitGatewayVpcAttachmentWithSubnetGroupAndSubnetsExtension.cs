...
public static class EC2TransitGatewayVpcAttachmentSubnetGroupWithSubnetsExtension
{
    public static TransitGatewayVpcAttachmentFactory WithEC2TransitGatewayVpcAttachmentSubnetGroupAndSubnets(this VPCFactory factory, string subnetName, Action<SubnetFactoryConfig> config)
    {
        var subnets = new List<IResultFactory>();
        var dbSubnetGroupFactory = factory.CreateSubFactory(() => new TransitGatewayVpcAttachmentFactory($"{subnetName} Subnet Group", x =>
        {
