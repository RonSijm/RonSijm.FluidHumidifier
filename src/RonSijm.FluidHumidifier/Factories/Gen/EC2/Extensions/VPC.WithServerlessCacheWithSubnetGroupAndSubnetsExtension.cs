...
public static class ElastiCacheServerlessCacheSubnetGroupWithSubnetsExtension
{
    public static ServerlessCacheFactory WithElastiCacheServerlessCacheSubnetGroupAndSubnets(this VPCFactory factory, string subnetName, Action<SubnetFactoryConfig> config)
    {
        var subnets = new List<IResultFactory>();
        var dbSubnetGroupFactory = factory.CreateSubFactory(() => new ServerlessCacheFactory($"{subnetName} Subnet Group", x =>
        {
