...
using RonSijm.FluidHumidifier.Factories.OpsWorksCM;
using RonSijm.FluidHumidifier.Factories.Custom.SubnetFactories;

namespace RonSijm.FluidHumidifier.Factories.EC2;

public static class OpsWorksCMServerSubnetGroupWithSubnetsExtension
{
    public static ServerFactory WithOpsWorksCMServerSubnetGroupAndSubnets(this VPCFactory factory, string subnetName, Action<SubnetFactoryConfig> config)
    {
        var subnets = new List<IResultFactory>();
        var dbSubnetGroupFactory = factory.CreateSubFactory(() => new ServerFactory($"{subnetName} Subnet Group", x =>
        {
